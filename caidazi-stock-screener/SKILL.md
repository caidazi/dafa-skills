...
用这个 skill 处理自然语言选股。公开 skill 必须调用服务端筛选 wrapper，不要在本地构造原始数据查询。筛选结果只是候选观察池，表示这些资产可能出现了更多值得继续验证的变化线索。
...
3. 重要限制或风险提示。
4. 数据时间。
5. 下一步建议：比较候选，或用资产研究、财经搜索、市场脉搏继续验证某个标的。

把结果表述为候选观察池，不要表述为推荐结论。
