...
test("MCP allowlist preserves the existing tools and adds only A-share minute routes", () => {
  ...
  assert.ok(ALLOWED_TOOL_SET.has("get_a_share_realtime_1m_price"));
  assert.ok(ALLOWED_TOOL_SET.has("get_a_share_history_1m_price"));
  assert.equal(ALLOWED_TOOL_SET.has("get_us_kline"), false);
});
