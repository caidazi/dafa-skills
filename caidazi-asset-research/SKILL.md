...
用这个 skill 处理单个标的实时行情、快速研究、深度研究和简单多标的比较。公开流程要尽量少澄清，优先通过 MCP wrapper 完成；深度研究需要用服务端工具补齐资金面、技术面、财务面和估值面，不在本地复刻分析模型。
...
1. 用户只问当前价格、涨跌幅、成交额、实时走势：使用实时行情流程。
...
4. 用户继续问"为什么涨跌"时，再进入快速研究或资讯检索，不要只凭价格走势推断原因。

## 快速研究流程

1. 识别标的。
2. 如用户关心"今天/现在/涨跌"，先调用 `get_real_time_record(symbol=...)`。
...
- `metrics`：按用户问题推断，默认 `["price", "valuation", "overview"]`；如果用户问深度比较，可加入 `"capital_flow"`、`"technical"`、`"financial"`。
